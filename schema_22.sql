...
\restrict Cn5GplMN8TRWIDhTaGRFKKIdSJyYfKH6bguiTa1dIeBEWKeF4KQa8SbRh3HsvxA
...
--
-- Name: project_item_status; Type: TYPE; Schema: public; Owner: -
--

CREATE TYPE public.project_item_status AS ENUM (
    'planned',
    'tentative',
    'confirmed',
    'waiting',
...
--
-- Name: project_item_type; Type: TYPE; Schema: public; Owner: -
--

CREATE TYPE public.project_item_type AS ENUM (
    'LIVE_HOLD',
    'TRAVEL_SEGMENT',
    'PROMO_SLOT',
    'RELEASE_MILESTONE',
...
--
-- Name: timeline_entry_type; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: timeline_lane; Type: TYPE; Schema: public; Owner: -
--

CREATE TYPE public.timeline_lane AS ENUM (
    'LIVE_HOLDS',
    'TRAVEL',
    'PROMO',
    'RELEASE',
...
CREATE FUNCTION public.current_auth_user_id() RETURNS uuid
...
--
-- Name: touch_timeline_entry_facets(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.touch_timeline_entry_facets() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
NEW.updated_at = now();
...
CREATE TABLE public.calendar_events (
    ...
    google_etag text,
    pending_action text,
    CONSTRAINT calendar_events_one_source_check CHECK ((((source_id IS NOT NULL) AND (user_source_id IS NULL)) OR ((source_id IS NULL) AND (user_source_id IS NOT NULL))))
);


--
-- Name: calendar_sync_states; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.project_items (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    project_id uuid NOT NULL,
    type public.project_item_type NOT NULL,
    kind text,
    title text NOT NULL,
    description text,
    start_at timestamp with time zone,
    ...
    tz text,
    status public.project_item_status DEFAULT 'planned'::public.project_item_status NOT NULL,
...
CREATE TABLE public.projects (
    ...
    created_by uuid DEFAULT public.current_auth_user_id(),
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL
);
...
--
-- Name: timeline_entries; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.timeline_entries AS
 SELECT id,
    project_id,
    type,
    kind,
    ...
    updated_at,
        CASE type
            WHEN 'LIVE_HOLD'::public.project_item_type THEN 'LIVE_HOLDS'::public.timeline_lane
            WHEN 'TRAVEL_SEGMENT'::public.project_item_type THEN 'TRAVEL'::public.timeline_lane
            WHEN 'PROMO_SLOT'::public.project_item_type THEN 'PROMO'::public.timeline_lane
            WHEN 'RELEASE_MILESTONE'::public.project_item_type THEN 'RELEASE'::public.timeline_lane
            WHEN 'LEGAL_ACTION'::public.project_item_type THEN 'LEGAL'::public.timeline_lane
            WHEN 'FINANCE_ACTION'::public.project_item_type THEN 'FINANCE'::public.timeline_lane
            ELSE NULL::public.timeline_lane
        END AS lane,
    '[]'::jsonb AS conflict_flags,
    NULL::smallint AS layout_row
   FROM public.project_items pi;


--
-- Name: VIEW timeline_entries; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON VIEW public.timeline_entries IS 'Denormalised project item view for timeline rendering.';


--
-- Name: timeline_entry_facets; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.timeline_entry_facets (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.timeline_items (
    ...
    entry_type public.timeline_entry_type DEFAULT 'task'::public.timeline_entry_type,
    entry_payload jsonb DEFAULT '{}'::jsonb,
    CONSTRAINT timeline_items_progress_percent_check CHECK (((progress_percent >= (0)::numeric) AND (progress_percent <= (100)::numeric)))
);


--
-- Name: user_calendar_sources; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: calendar_events calendar_events_unique_event; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.calendar_events
    ADD CONSTRAINT calendar_events_unique_event UNIQUE NULLS NOT DISTINCT (source_id, user_source_id, event_id);


--
-- Name: calendar_events calendar_events_unique_user_event; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: calendar_sync_states calendar_sync_states_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: calendar_sync_states calendar_sync_states_user_source_unique; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: calendar_watch_channels calendar_watch_channels_channel_unique; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: calendar_watch_channels calendar_watch_channels_user_source_unique; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: timeline_entry_facets timeline_entry_facets_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.timeline_entry_facets
    ADD CONSTRAINT timeline_entry_facets_pkey PRIMARY KEY (id);


--
-- Name: timeline_entry_facets timeline_entry_facets_timeline_item_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.timeline_entry_facets
    ADD CONSTRAINT timeline_entry_facets_timeline_item_id_key UNIQUE (timeline_item_id);
...
--
-- Name: user_calendar_sources user_calendar_sources_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: user_calendar_sources user_calendar_sources_user_id_calendar_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.user_calendar_sources
    ADD CONSTRAINT user_calendar_sources_user_id_calendar_id_key UNIQUE (user_id, calendar_id);
...
--
-- Name: calendar_events_assigned_project_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: calendar_events_ignore_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: calendar_events_origin_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: calendar_events_source_id_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: calendar_events_sync_status_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: calendar_events_user_events_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX calendar_events_user_events_idx ON public.calendar_events USING btree (user_source_id, start_at) WHERE (user_source_id IS NOT NULL);


--
-- Name: calendar_events_user_source_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX calendar_events_user_source_id_idx ON public.calendar_events USING btree (user_source_id);


--
-- Name: calendar_events_user_source_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: calendar_sync_states_user_source_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: calendar_watch_channels_user_source_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: project_items_labels_gin_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX project_items_labels_gin_idx ON public.project_items USING gin (labels);


--
-- Name: project_items_project_type_status_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX project_items_project_type_status_idx ON public.project_items USING btree (project_id, type, status);


--
-- Name: project_items_schedule_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX project_items_schedule_idx ON public.project_items USING btree (project_id, start_at);


--
-- Name: project_items_schedule_range_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX project_items_schedule_range_idx ON public.project_items USING gist (tstzrange(start_at, end_at));
...
--
-- Name: timeline_entry_facets_type_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX timeline_entry_facets_type_idx ON public.timeline_entry_facets USING btree (type);


--
-- Name: timeline_items_entry_type_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX timeline_items_entry_type_idx ON public.timeline_items USING btree (entry_type, project_id, starts_at);
...
--
-- Name: timeline_items_project_range_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX timeline_items_project_range_idx ON public.timeline_items USING btree (project_id, starts_at, ends_at);
...
--
-- Name: user_calendar_sources_account_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX user_calendar_sources_account_id_idx ON public.user_calendar_sources USING btree (account_id);


--
-- Name: user_calendar_sources_account_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: user_calendar_sources_user_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX user_calendar_sources_user_id_idx ON public.user_calendar_sources USING btree (user_id);


--
-- Name: user_calendar_sources_user_idx; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: calendar_sync_states calendar_sync_states_set_updated_at; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: calendar_watch_channels calendar_watch_channels_set_updated_at; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: project_items project_items_set_updated_at; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: timeline_entry_facets trigger_touch_timeline_entry_facets; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trigger_touch_timeline_entry_facets BEFORE UPDATE ON public.timeline_entry_facets FOR EACH ROW EXECUTE FUNCTION public.touch_timeline_entry_facets();


--
-- Name: user_calendar_sources user_calendar_sources_set_updated_at; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: calendar_events calendar_events_assigned_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: calendar_events calendar_events_item_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: calendar_events calendar_events_source_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: calendar_events calendar_events_user_source_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: calendar_sync_states calendar_sync_states_user_source_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: calendar_watch_channels calendar_watch_channels_user_source_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: project_assignment_rules project_assignment_rules_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: project_assignment_rules project_assignment_rules_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: project_email_link_overrides project_email_link_overrides_project_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: project_email_link_overrides project_email_link_overrides_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: project_items project_items_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
ALTER TABLE ONLY public.timeline_dependencies
    ADD CONSTRAINT timeline_dependencies_from_item_id_fkey FOREIGN KEY (from_item_id) REFERENCES public.project_items(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.timeline_dependencies
    ADD CONSTRAINT timeline_dependencies_to_item_id_fkey FOREIGN KEY (to_item_id) REFERENCES public.project_items(id) ON DELETE CASCADE;


--
-- Name: timeline_entry_facets timeline_entry_facets_timeline_item_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.timeline_entry_facets
    ADD CONSTRAINT timeline_entry_facets_timeline_item_id_fkey FOREIGN KEY (timeline_item_id) REFERENCES public.timeline_items(id) ON DELETE CASCADE;
...
--
-- Name: user_calendar_sources user_calendar_sources_account_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.user_calendar_sources
    ADD CONSTRAINT user_calendar_sources_account_id_fkey FOREIGN KEY (account_id) REFERENCES public.oauth_accounts(id) ON DELETE CASCADE;


--
-- Name: user_calendar_sources user_calendar_sources_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.user_calendar_sources
    ADD CONSTRAINT user_calendar_sources_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
--
-- Name: calendar_events calendar_events_project_member_insert; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY calendar_events_project_member_insert ON public.calendar_events FOR INSERT WITH CHECK (public.is_project_member(( SELECT project_sources.project_id
   FROM public.project_sources
  WHERE (project_sources.id = calendar_events.source_id))));


--
-- Name: calendar_events calendar_events_project_member_modify; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY calendar_events_project_member_modify ON public.calendar_events FOR UPDATE USING (public.is_project_member(( SELECT project_sources.project_id
   FROM public.project_sources
  WHERE (project_sources.id = calendar_events.source_id)))) WITH CHECK (public.is_project_member(( SELECT project_sources.project_id
   FROM public.project_sources
  WHERE (project_sources.id = calendar_events.source_id))));


--
-- Name: calendar_events calendar_events_project_member_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY calendar_events_project_member_select ON public.calendar_events FOR SELECT USING (public.is_project_member(( SELECT project_sources.project_id
   FROM public.project_sources
  WHERE (project_sources.id = calendar_events.source_id))));


--
-- Name: calendar_events calendar_events_service_role_all; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: calendar_events calendar_events_user_source_delete; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY calendar_events_user_source_delete ON public.calendar_events FOR DELETE USING (((user_source_id IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM public.user_calendar_sources
  WHERE ((user_calendar_sources.id = calendar_events.user_source_id) AND (user_calendar_sources.user_id = auth.uid()))))));


--
-- Name: calendar_events calendar_events_user_source_insert; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY calendar_events_user_source_insert ON public.calendar_events FOR INSERT WITH CHECK (((user_source_id IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM public.user_calendar_sources
  WHERE ((user_calendar_sources.id = calendar_events.user_source_id) AND (user_calendar_sources.user_id = auth.uid()))))));


--
-- Name: calendar_events calendar_events_user_source_update; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY calendar_events_user_source_update ON public.calendar_events FOR UPDATE USING (((user_source_id IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM public.user_calendar_sources
  WHERE ((user_calendar_sources.id = calendar_events.user_source_id) AND (user_calendar_sources.user_id = auth.uid()))))));


--
-- Name: calendar_events calendar_events_user_source_view; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY calendar_events_user_source_view ON public.calendar_events FOR SELECT USING (((user_source_id IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM public.user_calendar_sources
  WHERE ((user_calendar_sources.id = calendar_events.user_source_id) AND (user_calendar_sources.user_id = auth.uid()))))));


--
-- Name: calendar_sync_states; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.calendar_sync_states ENABLE ROW LEVEL SECURITY;

--
-- Name: calendar_sync_states calendar_sync_states_owner_modify; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY calendar_sync_states_owner_modify ON public.calendar_sync_states USING ((auth.role() = 'service_role'::text)) WITH CHECK ((auth.role() = 'service_role'::text));


--
-- Name: calendar_sync_states calendar_sync_states_owner_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: calendar_watch_channels; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.calendar_watch_channels ENABLE ROW LEVEL SECURITY;

--
-- Name: calendar_watch_channels calendar_watch_channels_owner_modify; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY calendar_watch_channels_owner_modify ON public.calendar_watch_channels USING ((auth.role() = 'service_role'::text)) WITH CHECK ((auth.role() = 'service_role'::text));


--
-- Name: calendar_watch_channels calendar_watch_channels_owner_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: oauth_sessions oauth_sessions_user_insert; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY oauth_sessions_user_insert ON public.oauth_sessions FOR INSERT TO authenticated WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY project_assignment_rules_user_modify ON public.project_assignment_rules USING ((auth.uid() = user_id)) WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY project_email_link_overrides_user_modify ON public.project_email_link_overrides USING ((auth.uid() = user_id)) WITH CHECK ((auth.uid() = user_id));
...
--
-- Name: project_items project_items_member_modify; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY project_items_member_modify ON public.project_items USING ((EXISTS ( SELECT 1
   FROM public.project_members pm
  WHERE ((pm.project_id = project_items.project_id) AND (pm.user_id = auth.uid()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.project_members pm
  WHERE ((pm.project_id = project_items.project_id) AND (pm.user_id = auth.uid())))));


--
-- Name: project_items project_items_member_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY project_items_member_select ON public.project_items FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.project_members pm
  WHERE ((pm.project_id = project_items.project_id) AND (pm.user_id = auth.uid())))));


--
-- Name: project_items project_items_service_role_full_access; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY project_items_service_role_full_access ON public.project_items TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY project_members_insert ON public.project_members FOR INSERT WITH CHECK (((auth.role() = 'service_role'::text) OR (EXISTS ( SELECT 1
   FROM public.projects p
  WHERE ((p.id = project_members.project_id) AND (p.created_by = auth.uid())))) OR public.is_project_member(project_id, ARRAY['owner'::public.project_member_role])));
...
--
-- Name: projects projects_authenticated_insert; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY projects_authenticated_insert ON public.projects FOR INSERT WITH CHECK (((auth.role() = 'service_role'::text) AND (created_by IS NOT NULL)));
...
--
-- Name: user_calendar_sources; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.user_calendar_sources ENABLE ROW LEVEL SECURITY;

--
-- Name: user_calendar_sources user_calendar_sources_own_delete; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY user_calendar_sources_own_delete ON public.user_calendar_sources FOR DELETE USING ((auth.uid() = user_id));


--
-- Name: user_calendar_sources user_calendar_sources_own_insert; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY user_calendar_sources_own_insert ON public.user_calendar_sources FOR INSERT WITH CHECK ((auth.uid() = user_id));


--
-- Name: user_calendar_sources user_calendar_sources_own_update; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY user_calendar_sources_own_update ON public.user_calendar_sources FOR UPDATE USING ((auth.uid() = user_id));


--
-- Name: user_calendar_sources user_calendar_sources_own_view; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY user_calendar_sources_own_view ON public.user_calendar_sources FOR SELECT USING ((auth.uid() = user_id));


--
-- Name: user_calendar_sources user_calendar_sources_owner_modify; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY user_calendar_sources_owner_modify ON public.user_calendar_sources USING (((auth.role() = 'service_role'::text) OR (auth.uid() = user_id))) WITH CHECK (((auth.role() = 'service_role'::text) OR (auth.uid() = user_id)));


--
-- Name: user_calendar_sources user_calendar_sources_owner_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: user_calendar_sources user_calendar_sources_service_role; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY user_calendar_sources_service_role ON public.user_calendar_sources USING ((auth.role() = 'service_role'::text));
...
\unrestrict Cn5GplMN8TRWIDhTaGRFKKIdSJyYfKH6bguiTa1dIeBEWKeF4KQa8SbRh3HsvxA
