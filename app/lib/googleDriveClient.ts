...
export async function getDriveAccount(
  ...
  return mapOAuthAccountRow(data as unknown as OAuthAccountRow);
}
...
export async function ensureDriveOAuthClient(
  ...
  const needsRefresh = !account.accessToken || !account.expiresAt || Date.parse(account.expiresAt) - TOKEN_EXPIRY_BUFFER_MS < Date.now();
